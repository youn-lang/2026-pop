...
DATA_FILE = "202606_202606_연령별인구현황_월간(1).csv"
...
def build_age_column_map(
    ...
    if not periods:
        raise ValueError("연령별 인구 열을 찾을 수 없습니다.")

    if not all(mapping[sex] for sex in ("계", "남", "여")):
        raise ValueError("전체·남성·여성 연령별 열을 모두 찾지 못했습니다.")
        ...
        records.append(
            {
                ...
                "전체": float(
                    row.get(column_map["계"].get(age), 0)
                ),
                "남성": float(
                    row.get(column_map["남"].get(age), 0)
                ),
                "여성": float(
                    row.get(column_map["여"].get(age), 0)
                ),
            }
        )
...
st.caption(
    "행정안전부 연령별 인구현황 CSV를 이용해 "
    "선택 지역의 연령별 인구 분포를 표시합니다."
)
...
try:
    ...
    period, age_column_map = build_age_column_map(
        tuple(df.columns)
    )
...
show_male = st.sidebar.checkbox(
    "남성",
    value=True,
)

show_female = st.sidebar.checkbox(
    "여성",
    value=True,
)
...
# 총인구 지표 계산
# --------------------------------------------------
total_col = find_total_column(
    df.columns.tolist(),
    period,
    "계",
)

male_col = find_total_column(
    df.columns.tolist(),
    period,
    "남",
)

female_col = find_total_column(
    df.columns.tolist(),
    period,
    "여",
)
...
# --------------------------------------------------
st.subheader(
    f"{selected_region} 인구 구조"
)

st.caption(
    f"기준: {period} · "
    f"행정구역 코드: {selected_code}"
)
...
summary_cols[0].metric(
    ...
    (
        f"{young_population / total_population * 100:.1f}%"
        if total_population
        else "0.0%"
    ),
)
...
summary_cols[1].metric(
    ...
    f"{working_age:,}명",
    (
        f"{working_age / total_population * 100:.1f}%"
        if total_population
        else "0.0%"
    ),
)
...
summary_cols[2].metric(
    ...
    f"{old_population:,}명",
    (
        f"{old_population / total_population * 100:.1f}%"
        if total_population
        else "0.0%"
    ),
)
